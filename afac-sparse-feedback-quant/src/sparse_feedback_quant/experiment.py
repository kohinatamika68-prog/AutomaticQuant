...
class Experiment:
    """Public experiment record with structured metadata."""
...
def validate_experiment_payload(payload: dict) -> list[str]:
    ...
    allowed_states = {item.value for item in SparseFeedbackState}
    for idx, row in enumerate(rows):
        ...
        seen_ids.add(exp_id)
        state = row.get("state")
        if state is not None and str(state) not in allowed_states:
